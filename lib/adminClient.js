...
class AdminClient {
  ...
  createTestUser() {
    return adminClient(this.config).then((client) => createTestUser(client, this.config.realm).then((newUser) => resetUserPassword(client, this.config.realm, newUser).then(() => newUser)));
  }
  ...
  updateTestUser() {
    return adminClient(this.config).then((client) =>
      this.findTestUser().then((user) => {
        ...
        return client.users.update(this.config.realm, user).then(() => "user updated");
      })
    );
  }
  ...
  setTestUserCustomerId() {
    return adminClient(this.config).then((client) =>
      this.findTestUser().then((user) => {
        ...
        return client.users.update(this.config.realm, user).then(() => "customerId added");
      })
    );
  }
  ...
  removeTestUserCustomerId() {
    return adminClient(this.config).then((client) =>
      this.findTestUser().then((user) => {
        ...
        return client.users.update(this.config.realm, user).then(() => "customerId removed");
      })
    );
  }
  ...
  addTestRoleToTestUser() {
    return this.findTestUser().then((user) =>
      this.authenticate()
        .then((token) => this.getRoleByName("TEST_ROLE").then((role) => this.request.addRole(user.id, role, token)))
  ...
  removeTestRoleFromTestUser() {
    return this.findTestUser().then((user) =>
      this.authenticate()
        .then((token) => this.getRoleByName("TEST_ROLE").then((role) => this.request.removeRoleFromUser(user.id, role, token)))
  ...
  getRoleByName(roleName) {
    return this.authenticate()
      ...
      .then((role) => (role ? Promise.resolve(role) : Promise.reject("role not found")));
  }
...
class KeyCloakAdminRequest {
  ...
  addRole(userId, role, token) {
    return this.doRequest("POST", `/admin/realms/${this.config.realm}/users/${userId}/role-mappings/realm`, token, [role]);
  }

  createRole(roleName, token) {
    return this.doRequest("POST", `/admin/realms/${this.config.realm}/roles`, token, {
      name: roleName,
    });
  }

  deleteRole(roleName, token) {
    return this.doRequest("DELETE", `/admin/realms/${this.config.realm}/roles/${roleName}`, token);
  }

  getRole(roleName, token) {
    return this.doRequest("GET", `/admin/realms/${this.config.realm}/roles/${roleName}`, token, null);
  }

  removeRoleFromUser(userId, role, token) {
    return this.doRequest("DELETE", `/admin/realms/${this.config.realm}/users/${userId}/role-mappings/realm`, token, [role]);
  }
