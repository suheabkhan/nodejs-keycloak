...
class KeyCloakService {
  ...
  static decodeAccessToken(accessToken) {
    ...
    let permissions = [];
    permissionsFromToken.forEach((permission) =>
      permissions.push({
  ...
  getAccessTokenWithPermissions(request) {
    let authzRequest = {};
    return this.keyCloak.grantManager.checkPermissions(authzRequest, request).then((grant) => {
  ...
  checkPermission(request, resource, scope) {
    ...
    return new Promise(function (resolve, reject) {
      keyCloak.grantManager.checkPermissions(authzRequest, request, (response) => resolve(response)).catch(reject);
    });
  }
